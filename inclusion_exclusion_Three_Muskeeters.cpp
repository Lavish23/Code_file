#include<bits/stdc++.h>
using namespace std;

bool isvowel(char ch)
{
    return ch=='a'||ch=='e'||ch=='i'||ch=='o'||ch=='u';
}

vector<string>subsets(string s)
{
    int sz=1<<s.size();
    vector<string>ans;
    for(int mask=0;mask<sz;++mask)
    {
        string subset;
        for(int bit=0;bit<s.size();++bit)
        {
            if(mask&(1<<bit))
            subset.push_back(s[bit]);
        }
        if (subset.size())
        ans.push_back(subset);
    }
    return ans;
}
int main()
{
    int t;
    cin>>t;
    while(t--)
    {
        int n;
        cin>>n;
        string str[n];
        for(int i=0;i<n;++i)
        cin>>str[i];
        unordered_map<string,int>ump;
        for(int i=0;i<n;++i)
        {
            set<char>distinct_vw;
            for(auto ch:str[i])
            {
                if(isvowel(ch))
                distinct_vw.insert(ch); // inserted all distinct vowel in set of ith string
            }
        

        // now we have to create all possible combo of the vowel and store their count in map
        string str_vowel;
        for(auto ch:distinct_vw)
        str_vowel.push_back(ch);
        vector<string>all_subsets=subsets(str_vowel);
        for(auto sub:all_subsets)
        {
            ump[sub]++;
        }
        }
        long long ans=0;
        for(auto it:ump)
        {
            if(it.second<3) continue;
            long long ct=it.second;
            long long ways=(ct)*(ct-1)*(ct-2)/6;
            if(it.first.size()%2==0)
            ans-=ways;
            else
            ans+=ways;
        }
            cout<<ans<<endl;
    }
    

}